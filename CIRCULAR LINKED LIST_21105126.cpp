...
Line 28 i.e. while(temp->next!=head){temp=temp->next;} establishes that the traversing element/variable has reached the first element.
...
7)Electronics Engineering :as my field is electronics and communication it was very difficult to think of practical applications but ther are some for eg if we build a mechatronics system like escalators we can make use of circular linked list in circular escalators.
*/
